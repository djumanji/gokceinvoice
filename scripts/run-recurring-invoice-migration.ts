...
async function runMigration() {
  ...
  try {
    console.log("Running recurring invoices migration...");
    ...
    await sql`
      ...
      ADD COLUMN IF NOT EXISTS recurring_invoice_id VARCHAR 
    ...
    await sql`
      CREATE INDEX IF NOT EXISTS idx_recurring_invoices_next_generation 
      ON recurring_invoices(next_generation_date) WHERE is_active = true;
    `;

    await sql`
      CREATE INDEX IF NOT EXISTS idx_recurring_invoice_items_recurring 
